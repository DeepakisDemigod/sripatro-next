...
const BirthPanchangUnified = () => {
  ...
  const yearsAD = Array.from({length: currentYear - 1899}, (_, i) => 1900 + i).reverse();
  ...
  const handleCalculate = async (e) => {
    ...
    try {
      ...
      if (mode === 'AD_TO_BS') {
        ...
        const bsObj = new NepaliDate(Number(bsYear), Number(bsMonth) - 1, Number(bsDay));
        adDate = bsObj.toJsDate ? bsObj.toJsDate() : bsObj.toJsDate(); // keep compatibility
        if (!adDate || isNaN(adDate.getTime())) throw new Error('Converted AD invalid');
        // attach time
      ...
      try {
        ...
        const res = await fetch(`https://api.sunrisesunset.io/json?lat=28.7041&lng=77.1025&date=${dateParam}`);
        if (res.ok) {
    ...
    <div className='min-h-screen bg-base-100 flex items-start justify-center py-8'>
      <div className='w-full max-w-5xl p-5'>
        <div className='bg-base-100/80  backdrop-blur-sm rounded-2xl shadow-2xl p-6 grid md:grid-cols-2 gap-6 transition-all'>
          <div>
            <a href='/' className='inline-flex items-center gap-2 text-sm text-base-700 hover:underline mb-3'>
            ...
            <a href='/date-converter' className='block mb-4'>
              <div className='rounded-xl p-4 border border-base-300 shadow-sm hover:shadow-md transition flex items-center justify-between'>
                <div>
                  ...
                  <p className='text-xs text-base-500'>change nepali date to indian date and indian date to nepali date.</p>
                </div>
                <div className='text-sm font-bold'>
                  <span className='text-2xl mr-2'>🇳🇵</span> ⇄ <span className='text-2xl ml-2 mr-2'>🇬🇧</span>
                </div>
              </div>
            </a>

            <h1 className='text-2xl font-bold mb-4 text-base-800'>{t('Ishwi Sambat to Panchang')}</h1>
            ...
            <form onSubmit={handleCalculate} className='space-y-5'>
                ...
                <>
                  ...
                  <div className='flex gap-3'>
                    ...
                    <div className='flex-1'>
                      ...
                      >
                        {Array.from({ length: daysInMonthAD(adYear, adMonth) }, (_, i) => {
                          const d = i + 1;
                          const ds = String(d).padStart(2, '0');
                          return <option key={ds} value={ds}>{ds}</option>;
                        })}
                      </select>
                      <div className='text-xs text-center mt-1 text-base-400'>{t('Day')}</div>
                    </div>
                    ...
                    <div className='flex-1'>
                      ...
                      <div className='text-xs text-center mt-1 text-base-400'>{t('Month')}</div>
                    </div>
                    ...
                    <div className='flex-1'>
                      <select
                        ...
                        className='w-full text-center py-3 rounded-xl border border-base-300 bg-base-100 shadow-sm focus:ring-2 focus:ring-red-300 transition'
                      >
                        {yearsAD.map(y => <option key={y} value={y}>{y}</option>)}
                      </select>
                      <div className='text-xs text-center mt-1 text-base-400'>{t('Year')}</div>
                    </div>
                  </div>
                </>
              ) : (
              ...
              <div>
                <label htmlFor='time' className='flex items-center gap-2 block text-sm font-medium text-base-600 mb-2'>
                  <Alarm size={18} />
                  ...
                  <div className='flex justify-between items-start'>
                    <div>
                      <h2 className='text-sm font-bold text-base-800'>
                        {t(`day.${panchang?.Day?.name_en_UK}`)}{', '}{englishDate}
                      </h2>
                      ...
                      {mode === 'AD_TO_BS' && bsConvertedDate && (
                        <div className='mt-2 text-sm font-semibold text-base-600'>
                          {bsConvertedDate.year} {BS_MONTHS_NEPALI[bsConvertedDate.month]} {bsConvertedDate.day}
                        </div>
                      )}
                  ...
                  <table className='table-auto w-full text-sm'>
                    <tbody>
                      <tr className="border-b border-base-300">
                        ...
                        <td className='py-2'>{t(`day.${panchang?.Day?.name_en_UK}`) || 'Not Available'}</td>
                      </tr>

                      <tr className="border-b border-base-300">
                        <th className='text-left py-2 pr-4'>{t('Paksh')}</th>
                        <td className='py-2'>{t(`paksha.${panchang?.Paksha?.name_en_IN}`) || 'Not Available'}</td>
                      </tr>
                      ...
                      <tr className="border-b border-base-300">
                        ...
                        <td className='py-2 flex items-center gap-2'>
                          ...
                          <div>{t(`tithi.${panchang?.Tithi?.name_en_IN}`) || 'Not Available'}</div>
                        </td>
                      </tr>

                      <tr className="border-b border-base-300">
                        <th className='text-left py-2 pr-4'>{t('Nakshatra')}</th>
                        <td className='py-2'>{t(`nakshatra.${panchang?.Nakshatra?.name_en_IN}`) || 'Not Available'}</td>
                      </tr>

                      <tr className="border-b border-base-300">
                        <th className='text-left py-2 pr-4'>{t('Rasi')}</th>
                        <td className='py-2'>{t(`rasi.${panchang?.Raasi?.name_en_UK}`) || 'Not Available'}</td>
                      </tr>

                      {nakshatraInfo && (
                        <>
                          <tr className="border-b border-base-300">
                            <th className='text-left py-2 pr-4'>{t('Syllables')}</th>
                            <td className='py-2'>{t(`syllables.${nakshatraInfo['first syllables']}`) || 'Not Available'}</td>
                          </tr>
                          <tr className="border-b border-base-300">
                            <th className='text-left py-2 pr-4'>{t('Gan')}</th>
                            <td className='py-2'>{t(`ganam.${nakshatraInfo.ganam}`) || 'Not Available'}</td>
                          </tr>
                          <tr className="border-b border-base-300">
                            <th className='text-left py-2 pr-4'>{t('Animal Sign')}</th>
                            <td className='py-2'>{t(`animal.${nakshatraInfo['animal sign']}`) || 'Not Available'}</td>
                          </tr>
                          <tr className="border-b border-base-300">
                            <th className='text-left py-2 pr-4'>{t('Deity')}</th>
                            <td className='py-2'>{t(`deity.${nakshatraInfo.Diety}`) || 'Not Available'}</td>
                          </tr>
                          <tr className="border-b border-base-300">
                            <th className='text-left py-2 pr-4'>{t('Best Direction')}</th>
                            <td className='py-2'>{t(`best_direction.${nakshatraInfo['best direction']}`) || 'Not Available'}</td>
                          </tr>
                        </>
                      )}
                      ...
                      <tr className="border-b border-base-300">
                        ...
                        <td className='py-2'>{t(`yoga.${panchang?.Yoga?.name_en_IN}`) || 'Not Available'}</td>
                      </tr>

                      <tr className="border-b border-base-300">
                        <th className='text-left py-2 pr-4'>{t('Karna')}</th>
                        <td className='py-2'>{t(`karna.${panchang?.Karna?.name_en_IN}`) || 'Not Available'}</td>
                      </tr>

                      <tr className="border-b border-base-300">
                        <th className='text-left py-2 pr-4'>{t('Sunrise')}</th>
                        <td className='py-2'>{t('timePeriod.Morning')} {sunData?.sunrise || 'Unknown'}</td>
                      </tr>

                      <tr className="border-b border-base-300">
                        <th className='text-left py-2 pr-4'>{t('Sunset')}</th>
                        <td className='py-2'>{t('timePeriod.Evening')} {sunData?.sunset || 'Unknown'}</td>
                      </tr>

                      {age && (
                        <tr>
                          <th className='text-left py-2 pr-4'>{t('Age')}</th>
                          <td className='py-2'>{age.years} {t('years and')} {age.months} {t('months')}</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
